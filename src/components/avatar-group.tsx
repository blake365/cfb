...
export function AvatarGroupComponent({
	...
	return (
		<div className='overflow-x-auto max-w-[calc(100vw-8rem)]'>
			<div className='flex items-center'>
				<div className='flex -space-x-4 sm:hover:space-x-2 hover:space-x-0 transition-all duration-300 ease-in-out'>
					{displayAvatars.map((avatar) => (
						<Link href={`/teams/${avatar.name}`} key={avatar.id}>
							<Avatar className='w-10 h-10 border border-background hover:z-10 bg-background hover:border-primary'>
